...
public class NsdVnfContributorV3 extends AbstractNsdContributorV3<Object> {
	...
	protected NsdVnfContributorV3(final NsLiveInstanceJpa nsLiveInstanceJpa, final NsScaleStrategyV3 nsScaleStrategy, final ServersRepositoryService serversJpa, final VnfPackageService vnfPackageService) {
	...
	public List<SclableResources<Object>> contribute(final NsdPackage bundle, final NsBlueprint parameters) {
		...
		final List<VnfPackage> vnfs = vnfsCopy.stream().map(x -> vnfPackageService.findByVnfdIdOpt(x.getVnfdId()).orElseThrow()).toList();
