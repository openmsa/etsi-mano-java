...
public class VnfInstanceServiceImpl implements VnfInstanceService {
	...
	public VnfInstance vnfLcmPatch(final VnfInstance vnfInstance, final String body, final @Nullable String ifMatch) {
		if ((ifMatch != null) && !(vnfInstance.getVersion() + "").equals(ifMatch)) {
	...
	@Override
	public <U> ResponseEntity<String> search(final MultiValueMap<String, String> requestParams, final Function<VnfInstance, U> mapper, final @Nullable String excludeDefaults, final Set<String> mandatoryFields, final Consumer<U> makeLink, final Class<U> frontClass) {
		final SearchParamBuilder<VnfInstance, U> params = SearchParamBuilder.of(VnfInstance.class, frontClass)
